...
trait Optimizer[O, S] {
  def rules[V]: List[OptimizationRule[O, S, V]]

  @tailrec
  final def optimize[V](term: AbstractSyntaxTree[O, S, V]): AbstractSyntaxTree[O, S, V] = {
    def applyRules(node: AbstractSyntaxTree[O, S, V]): AbstractSyntaxTree[O, S, V] =
      rules[V].reduce(_ orElse _).lift(node).getOrElse(node)
...
object Optimize {
  def replaceConstantsRule[O, S, V](using eval: Evaluate[O, S]): OptimizationRule[O, S, V] = {
