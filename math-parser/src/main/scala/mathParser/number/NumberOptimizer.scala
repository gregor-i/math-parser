...
class NumberOptimizer[S: Number](using evaluate: mathParser.Evaluate[NumberOperator, S])
  ...
  private def identities[V]: OptimizationRule[NumberOperator, S, V] = {
    ...
    {
      case UnitaryNode(Minus, UnitaryNode(Minus, child))      => child
      ...
      case BinaryNode(Plus, left, UnitaryNode(Minus, child))  => left - child
      case BinaryNode(Minus, left, UnitaryNode(Minus, child)) => left + child
