...
object NumberSyntax {
  def neg[S, V](t: AST[NumberOperator, S, V]): AST[NumberOperator, S, V]  = UnitaryNode(Minus, t)
  ...
  def sqrt[S: Number, V](t: AST[NumberOperator, S, V]): AST[NumberOperator, S, V] =
    BinaryNode(Power, t, ConstantNode(Number.oneHalf[S]))
